...
class MathematicalModel(Estimator):
    ...
    def __init__(self):
        super(MathematicalModel, self).__init__()
        self._evaluations = 0
        self._calls = 0
    ...
    def estimate(
        ...
        **kwargs
        ...
        #Correctness marks
            ...
            correct_marks= (x1*((5-current_level)/5) *alpha*(val*(numpy.sqrt(numpy.exp((val**2)/37)-1)/5)))-(x2*((current_level)/5)*alpha*(val*(numpy.sqrt(numpy.exp((val**2)/37)-1)/5)))
            ...
            time_marks = (x1*((5-current_level)/5)*beta *(numpy.exp(-time_taken/(max_time*modval))))+(x2*((current_level)/5)*beta*(numpy.exp(-max_time/time_taken*modval)))
            # if correctness==-1:
