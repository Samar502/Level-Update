...
class Simulator:
    ...
    def simulate(
        ...
        >>> from catsim.estimation import  MathematicalModel
        ...
        for current_examinee, true_theta in enumerate(self.examinees):
            ...
            if verbose:
                pbar.update()
            # addding intial level in est theta
            # est_theta=will take input for now,initial level
            est_theta = self._initializer.initialize(current_examinee)
            #est_theta= float(input('Enter a float: '))
            ...
            while not self._stopper.stop(current_examinee):
                ...
                # logistic function
                # response= will take as input everytime
                response = irt.icc(
                ...
                # ---------------------------------
                current_level=true_theta;
                ques_level= self.items[selected_item, 1]
                if response==True:
                    correctness=1
                else:
                    correctness=-1
                # print(self.items[selected_item, 1])
                #max_time=30
                #time_taken=40
                #alpha=0.8
                #beta=0.2

                est_theta = self._estimator.estimate(self._estimations[current_examinee][-1],self.items[selected_item, 1],correctness,30,10,0.6,0.2)

                # -----------------------
                # estimate the new theta using the given estimator
                """if choice ==1:
                    est_theta = self._estimator.estimate(current_examinee)
                else :#for mathematical model
                    est_theta = self._estimator.estimate(self._estimations[current_examinee][-1],self.items[selected_item, 1],correctness,30,10,0.8,0.2)"""#,ques_level,correctness,max_time,time_taken,alpha,beta)))
                ...
                # print(self._estimations[current_examinee][-1])
                self._estimations[current_examinee].append(est_theta)
